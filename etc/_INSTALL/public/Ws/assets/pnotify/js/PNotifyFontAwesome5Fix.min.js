...
_createClass(SvelteComponent,[{key:"$destroy",value:function $destroy(){destroy_component(this,1);this.$destroy=noop;}},{key:"$on",value:function $on(type,callback){var callbacks=this.$$.callbacks[type]||(this.$$.callbacks[type]=[]);callbacks.push(callback);return function(){var index=callbacks.indexOf(callback);if(index!==-1)callbacks.splice(index,1);};}},{key:"$set",value:function $set($$props){if(this.$$set&&!is_empty($$props)){this.$$.skip_bound=true;this.$$set($$props);this.$$.skip_bound=false;}}}]);return SvelteComponent;}();var position="PrependContainer";var defaults={};function instance($$self,$$props,$$invalidate){var _$$props$self=$$props.self,self=_$$props$self===void 0?null:_$$props$self;var _updatingIcon=false;var _updatingSticker=false;var _oldIconProp=self.icon;var _oldIconValue=self.icon===true?self.getIcon(self.type):self.icon;var _oldSticker="".concat(self.getIcon("sticker")," ").concat(self.hide?self.getIcon("unstuck"):self.getIcon("stuck"));var newIconProp;var newIconValue;var newSticker;var removeIconHandler=self.on("pnotify:update",function(){if(_updatingIcon){return;}
newIconProp=self.icon;newIconValue=self.icon===true?self.getIcon(self.type):self.icon;if(newIconValue!==_oldIconValue&&(typeof newIconValue==="string"&&newIconValue.match(/(^| )fa[srlb]($| )/)||typeof _oldIconValue==="string"&&_oldIconValue.match(/(^| )fa[srlb]($| )/))){$$invalidate(0,self.icon=false,self);_updatingIcon=true;tick().then(function(){$$invalidate(0,self.icon=newIconProp,self);_updatingIcon=false;_oldIconProp=newIconProp;_oldIconValue=newIconValue;});}else{_oldIconProp=newIconProp;_oldIconValue=newIconValue;}});var removeStickerHandler=self.on("pnotify:update",function(){if(_updatingSticker){return;}
newSticker="".concat(self.getIcon("sticker")," ").concat(self.hide?self.getIcon("unstuck"):self.getIcon("stuck"));if(self.sticker&&newSticker!==_oldSticker&&typeof newSticker==="string"&&newSticker.match(/(^| )fa[srlb]($| )/)){$$invalidate(0,self.sticker=false,self);_updatingSticker=true;tick().then(function(){$$invalidate(0,self.sticker=true,self);_updatingSticker=false;_oldSticker=newSticker;});}else{_oldSticker=newSticker;}});onDestroy(function(){removeIconHandler&&removeIconHandler();removeStickerHandler&&removeStickerHandler();});$$self.$$set=function($$props){if("self"in $$props)$$invalidate(0,self=$$props.self);};return[self];}
var Font_awesome5_fix=function(_SvelteComponent){_inherits(Font_awesome5_fix,_SvelteComponent);var _super=_createSuper(Font_awesome5_fix);function Font_awesome5_fix(options){var _this;_classCallCheck(this,Font_awesome5_fix);_this=_super.call(this);init(_assertThisInitialized(_this),options,instance,null,safe_not_equal,{self:0});return _this;}
return Font_awesome5_fix;}(SvelteComponent);exports.default=Font_awesome5_fix;exports.defaults=defaults;exports.position=position;Object.defineProperty(exports,'__esModule',{value:true});})));