...
_createClass(SvelteComponent,[{key:"$destroy",value:function $destroy(){destroy_component(this,1);this.$destroy=noop;}},{key:"$on",value:function $on(type,callback){var callbacks=this.$$.callbacks[type]||(this.$$.callbacks[type]=[]);callbacks.push(callback);return function(){var index=callbacks.indexOf(callback);if(index!==-1)callbacks.splice(index,1);};}},{key:"$set",value:function $set($$props){if(this.$$set&&!is_empty($$props)){this.$$.skip_bound=true;this.$$set($$props);this.$$.skip_bound=false;}}}]);return SvelteComponent;}();var window_1=globals.window;function create_fragment(ctx){var mounted;var dispose;return{c:noop,m:function m(target,anchor){if(!mounted){dispose=listen(window_1,"resize",ctx[3]);mounted=true;}},p:noop,i:noop,o:noop,d:function d(detaching){mounted=false;dispose();}};}
var position="PrependContainer";var defaults={swipeDismiss:true};function instance($$self,$$props,$$invalidate){var _$$props$self=$$props.self,self=_$$props$self===void 0?null:_$$props$self;var _$$props$swipeDismiss=$$props.swipeDismiss,swipeDismiss=_$$props$swipeDismiss===void 0?defaults.swipeDismiss:_$$props$swipeDismiss;var origXY=null;var diffXY=null;var noticeWidthHeight=null;var noticeOpacity=null;var csspos="left";var direction="X";var span="Width";var windowInnerWidth=window.innerWidth;var offs=[];onMount(function(){offs=[self.on("touchstart",function(e){if(!swipeDismiss){return;}
var stack=self.stack;if(stack){switch(stack.dir1){case"up":case"down":csspos="left";direction="X";span="Width";break;case"left":case"right":csspos="top";direction="Y";span="Height";break;}}
origXY=e.touches[0]["screen".concat(direction)];noticeWidthHeight=self.refs.elem["scroll".concat(span)];noticeOpacity=window.getComputedStyle(self.refs.elem)["opacity"];$$invalidate(1,self.refs.container.style[csspos]=0,self);}),self.on("touchmove",function(e){if(!origXY||!swipeDismiss){return;}
var curXY=e.touches[0]["screen".concat(direction)];diffXY=curXY-origXY;var opacity=(1-Math.abs(diffXY)/noticeWidthHeight)*noticeOpacity;$$invalidate(1,self.refs.elem.style.opacity=opacity,self);$$invalidate(1,self.refs.container.style[csspos]="".concat(diffXY,"px"),self);}),self.on("touchend",function(){if(!origXY||!swipeDismiss){return;}
self.refs.container.classList.add("pnotify-mobile-animate-left");if(Math.abs(diffXY)>40){var goLeft=diffXY<0?noticeWidthHeight*-2:noticeWidthHeight*2;$$invalidate(1,self.refs.elem.style.opacity=0,self);$$invalidate(1,self.refs.container.style[csspos]="".concat(goLeft,"px"),self);self.close();}else{self.refs.elem.style.removeProperty("opacity");self.refs.container.style.removeProperty(csspos);}
origXY=null;diffXY=null;noticeWidthHeight=null;noticeOpacity=null;}),self.on("touchcancel",function(){if(!origXY||!swipeDismiss){return;}
self.refs.elem.style.removeProperty("opacity");self.refs.container.style.removeProperty(csspos);origXY=null;diffXY=null;noticeWidthHeight=null;noticeOpacity=null;}),self.on("pnotify:afterClose",function(){if(!swipeDismiss){return;}
self.refs.elem.style.removeProperty("opacity");self.refs.container.style.removeProperty("left");self.refs.container.style.removeProperty("top");})];});onDestroy(function(){offs.forEach(function(off){return off();});});var resize_handler=function resize_handler(){return $$invalidate(0,windowInnerWidth=window.innerWidth);};$$self.$$set=function($$props){if("self"in $$props)$$invalidate(1,self=$$props.self);if("swipeDismiss"in $$props)$$invalidate(2,swipeDismiss=$$props.swipeDismiss);};$$self.$$.update=function(){if($$self.$$.dirty&3){{var stack=self.stack;if(stack){if(windowInnerWidth<=480){if(!("_m_spacing1"in stack)){stack._m_spacing1=stack.spacing1;stack._m_firstpos1=stack.firstpos1;stack._m_spacing2=stack.spacing2;stack._m_firstpos2=stack.firstpos2;stack.spacing1=0;stack.firstpos1=0;stack.spacing2=0;stack.firstpos2=0;stack.queuePosition();}}else{if("_m_spacing1"in stack){stack.spacing1=stack._m_spacing1;delete stack._m_spacing1;stack.firstpos1=stack._m_firstpos1;delete stack._m_firstpos1;stack.spacing2=stack._m_spacing2;delete stack._m_spacing2;stack.firstpos2=stack._m_firstpos2;delete stack._m_firstpos2;stack.queuePosition();}}}}}};return[windowInnerWidth,self,swipeDismiss,resize_handler];}
var Mobile=function(_SvelteComponent){_inherits(Mobile,_SvelteComponent);var _super=_createSuper(Mobile);function Mobile(options){var _this;_classCallCheck(this,Mobile);_this=_super.call(this);init(_assertThisInitialized(_this),options,instance,create_fragment,safe_not_equal,{self:1,swipeDismiss:2});return _this;}
return Mobile;}(SvelteComponent);exports.default=Mobile;exports.defaults=defaults;exports.position=position;Object.defineProperty(exports,'__esModule',{value:true});})));