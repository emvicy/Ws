...
animSpeed=animSpeed/1000;if(self.refs.elem.style.animationDuration!=="".concat(animSpeed,"s")){$$invalidate(0,self.refs.elem.style.animationDuration="".concat(animSpeed,"s"),self);}}});function animateIn(callback,immediate){self.setAnimating("in");var off;var finished=function finished(event){if(event&&self.refs.elem&&event.target!==self.refs.elem){return;}
off();self.setAnimatingClass("pnotify-in animated");if(callback){callback.call();}
self.setAnimating(false);};off=self.on("animationend",finished);if(immediate){finished();}else{self.setAnimatingClass("pnotify-in animated ".concat(inClass||outClass));}}
function animateOut(callback,immediate){self.setAnimating("out");var off;var finished=function finished(event){if(event&&self.refs.elem&&event.target!==self.refs.elem){return;}
off();self.setAnimatingClass("animated");if(callback){callback.call();}
if(self.setAnimating){self.setAnimating(false);}};off=self.on("animationend",finished);if(immediate){finished();}else{self.setAnimatingClass("pnotify-in animated ".concat(outClass||inClass));}}
...
var Animate=function(_SvelteComponent){_inherits(Animate,_SvelteComponent);var _super=_createSuper(Animate);function Animate(options){var _this;_classCallCheck(this,Animate);_this=_super.call(this);init(_assertThisInitialized(_this),options,instance,null,safe_not_equal,{self:0,inClass:1,outClass:2});return _this;}
return Animate;}(SvelteComponent);exports.default=Animate;exports.defaults=defaults;exports.position=position;Object.defineProperty(exports,'__esModule',{value:true});})));