...
class Chat extends Application
{
    ...
    {
        $id = $oConnection->getClientId();
        $this->clients[$id] = $oConnection;
    ...
    public function onData(string $data, Connection $oConnection): void
    {
        try {
            ...
            $clientId = $oConnection->getClientId();
    ...
    {
        $actionName = 'action' . ucfirst($aData['action']);
        $message = 'System Message: ' . $aData['data'] ?? '';
